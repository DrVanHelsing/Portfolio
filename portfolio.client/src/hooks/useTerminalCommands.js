...
function cmdMan({ args, addLines }) {
  if (!args[0]) {
    addLines([
      { type: 'rdy', text: '  tree                    full filesystem (start here)' },
      { type: 'rdy', text: '  projects                list all projects' },
      { type: 'rdy', text: '  cat <file|alias>        read a file   (cat resume.md, cat geology)' },
      { type: 'rdy', text: '  cd  <path|/page|slug>   change dir or jump to a page  (cd /about)' },
      { type: 'dim', text: '  ls [path]   grep <q>    list contents · search portfolio' },
      { type: 'blank' },
      { type: 'sym', text: '  /chat                   multi-turn AI chat' },
      { type: 'sym', text: '  summarize <page|proj>   streamed AI summary' },
      { type: 'dim', text: '  …or just type any question — it goes to the AI.' },
      { type: 'blank' },
      { type: 'dim', text: '  whoami · env · history · pwd · clear' },
      { type: 'dim', text: '  github · linkedin · email' },
      { type: 'dim', text: '  mode ai | mode dev      switch to AI Chat mode' },
      { type: 'dim', text: '  help [cmd]              this menu, or a single manual page' },
      { type: 'blank' },
      { type: 'dim', text: '  Tab autocompletes · ↑/↓ for history' },
      { type: 'blank' },
    ]);
    return;
  }
